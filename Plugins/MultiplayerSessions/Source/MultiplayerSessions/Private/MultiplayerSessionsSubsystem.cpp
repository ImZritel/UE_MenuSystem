...
void UMultiplayerSessionsSubsystem::CreateSession(int32_t NumPublicConnections, FString MatchType) {
    ...
    if (ExistingSession) {
        ...
        DestroySession(); // was: SessionInterface->DestroySession(NAME_GameSession); but may cause errors with hosting new session
    }
    ...
    LastSessionSettings->BuildUniqueId = 42;
...
void UMultiplayerSessionsSubsystem::FindSessions(int32_t MaxSearchResults) {
    ...
    LastSessionSearch = MakeShareable(new FOnlineSessionSearch()); // search settings ptr
	...
	if (!SessionInterface->FindSessions(*LocalPlayer->GetPreferredUniqueNetId(), LastSessionSearch.ToSharedRef())) {
        SessionInterface->ClearOnFindSessionsCompleteDelegate_Handle(FindSessionsCompleteDelegateHandle);
        MultiplayerOnFindSessionsComplete.Broadcast({}, false); // MultiplayerOnFindSessionsComplete.Broadcast(TArray<FOnlineSessionSearchResult, false);
    } // else call UMultiplayerSessionsSubsystem::OnFindSessionsComplete delegate
}
void UMultiplayerSessionsSubsystem::JoinSession(const FOnlineSessionSearchResult& SessionResult) {
    ...
    if(!SessionInterface->JoinSession(*LocalPlayer->GetPreferredUniqueNetId(), NAME_GameSession, SessionResult)) {
        SessionInterface->ClearOnJoinSessionCompleteDelegate_Handle(JoinSessionCompleteDelegateHandle);
        MultiplayerOnJoinSessionComplete.Broadcast(EOnJoinSessionCompleteResult::UnknownError);
    } // else call OnJoinSessionComplete delegate
...
void UMultiplayerSessionsSubsystem::DestroySession() {
    ...
    if(!SessionInterface->DestroySession(NAME_GameSession)) {
        SessionInterface->ClearOnDestroySessionCompleteDelegate_Handle(DestroySessionCompleteDelegateHandle);
        MultiplayerOnDestroySessionComplete.Broadcast(false);
    } // else call callback function OnDestroySessionComplete
}
